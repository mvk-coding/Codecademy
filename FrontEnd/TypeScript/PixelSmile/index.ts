...
/**
 * Stores a dot in the image data.
...
/**
 * Stores a horizontal line in the image data.
...
/**
 * Stores a vertical line in the image data.
 * @param x - The horizontal starting position.
 * @param y - The vertical starting position.
 */
...
/**
 * Attempt to store a crudely image of a circle in the 
 * image data.
 * Function is not perfect yet. 
 * 
 * @param x1 - The horizontal middle of the circle.
 * @param y1 - The vertical middle of the circle.
...
function drawCircle(x: number, y: number, radius: number){
  if(radius === 0){
    ...
    for(let i = 0; i < (2 * Math.PI); i += Math.PI/360) {
      let x1 = x + Math.round(radius * Math.cos(i));
      let y1 = y + Math.round(radius * Math.sin(i));
      drawDot(x1,y1);
    }
  }
}
...
/**
 * Stores a shape of a rectangle in the image data.
 * @param x - The horizontal starting position.
 * @param y - The vertical starting position.
